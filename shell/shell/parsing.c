...
{
	// Your code here
	// Verificacion de que el primer byte del argumento contiene '$'
	if (arg[0] != '$' || !arg) {
		return arg;
	}

	// Caso especial: pseudo-variable "$?"
	if (strcmp(arg, "$?") == 0) {
		char buffer[16];
	...
	// Obtengo el argumento sin '$' y luego obtengo su variable
	char *argumento = arg + 1;
	char *valor = getenv(argumento);
	if (!valor) {
		return strdup("");
	}
